...
"""
    ...
    async def _execute_writer(
        ...
        if files_to_generate:
            ...
            # ============================================================
            # ESSENTIAL FILES: Ensure all required config files exist
            # This adds tsconfig.node.json, tailwind.config.js, etc.
            # if they weren't generated by the AI.
            # ============================================================
            try:
                from app.utils.config_templates import get_all_essential_files

                # Detect project type from files
                project_type = "vite-react"  # Default
                created_paths = [f.get("path", "") if isinstance(f, dict) else str(f) for f in context.files_created]

                if any("next.config" in p for p in created_paths):
                    project_type = "nextjs"
                elif any("requirements.txt" in p or "main.py" in p for p in created_paths):
                    project_type = "fastapi"
                elif any("pom.xml" in p or "build.gradle" in p for p in created_paths):
                    project_type = "spring"
                elif any("vite.config" in p or "package.json" in p for p in created_paths):
                    project_type = "vite-react"

                essential_files = get_all_essential_files(project_type)

                # Find missing essential files
                missing_essential = []
                for file_path, content in essential_files.items():
                    # Check if file exists (match by filename, ignoring directory)
                    filename = file_path.split("/")[-1]
                    file_exists = any(
                        (p.endswith(filename) or p.endswith(file_path))
                        for p in created_paths
                    )
                    if not file_exists:
                        missing_essential.append((file_path, content))

                if missing_essential:
                    logger.info(f"[EssentialFiles] Adding {len(missing_essential)} missing essential files: {[f[0] for f in missing_essential]}")

                    yield OrchestratorEvent(
                        type=EventType.STATUS,
                        data={
                            "message": f"Adding {len(missing_essential)} essential config files...",
                            "missing_count": len(missing_essential),
                            "phase": "essential_files"
                        }
                    )

                    for file_path, content in missing_essential:
                        try:
                            save_success = await self.save_file(
                                project_id=context.project_id,
                                file_path=file_path,
                            ...
                            if save_success:
                                ...
                                logger.info(f"[EssentialFiles] ✓ Added {file_path}")
                            ...
                            logger.warning(f"[EssentialFiles] Failed to add {file_path}: {save_err}")

                    logger.info(f"[EssentialFiles] ✓ Added {len(missing_essential)} essential files")
                else:
                    logger.info("[EssentialFiles] ✓ All essential config files present")
...
💡 **Pro Tip:** Read the generated code to understand patterns - modify and experiment to learn faster."""
    ...
    async def _execute_documenter(
        ...
        if is_academic:
            ...
            # Non-academic users: Explain why and generate basic docs
            skip_reason = ""
            if not is_student and not is_pro_plan:
                skip_reason = f"Academic documents require student/faculty role with PRO subscription. Your role: {user_role or 'unknown'}, plan: {subscription_tier}"
            elif not is_student:
                skip_reason = f"Academic documents require student or faculty role. Your current role: {user_role or 'unknown'}. Update your profile to access SRS, PPT, and Project Report."
            elif not is_pro_plan:
                skip_reason = f"Academic documents require PRO subscription. Your plan: {subscription_tier}. Upgrade to PRO for full document generation."

            logger.info(f"[Documenter] SKIPPING academic docs: {skip_reason}")

            # Send DOCUMENTS_SKIPPED event so frontend updates the Documents panel properly
            yield OrchestratorEvent(
                type=EventType.STATUS,
                data={
                    "message": skip_reason,
            ...
            # Generate basic documentation (Dockerfile, docker-compose)
            logger.info(f"[Documenter] Generating basic docs for non-academic user (tier={subscription_tier}, role={user_role})")
            ...
            yield OrchestratorEvent(
                type=EventType.STATUS,
                data={
                    "message": "Upgrade to PRO for academic documents (SRS, PPT, VIVA Q&A)",
                    "upgrade_required": not is_pro_plan,
                    "role_required": not is_student,
    ...
    def _generate_dockerfile(
        ...
        if has_next:
            return '''# Next.js Development Dockerfile
FROM node:18-alpine
WORKDIR /app
