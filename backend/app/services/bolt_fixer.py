...
🚨 LOMBOK AWARENESS (CRITICAL FOR JAVA):
- Classes with @Data annotation ALREADY have getters/setters generated by Lombok
- If class has @Data and still shows "cannot find symbol: method getX()", check if Lombok is in pom.xml
- Do NOT add explicit getter methods if @Data is present - just ensure @Data annotation exists
- Do NOT add duplicate field declarations - check existing fields in FILE CONTENT before adding
- If you see "variable X is already defined" error, you've added a duplicate field - REMOVE IT

🚨 AVOID DUPLICATE FIELDS:
- BEFORE adding a field, check if it already exists in the provided FILE CONTENT
- NEVER add a field that is already declared in the class
- If getter is missing but field exists, add ONLY the getter method (not the field)
- For Lombok @Data classes, missing getters means Lombok config issue, not missing field

🚨 PACKAGE CONSISTENCY (CRITICAL FOR JAVA):
- If you see "incompatible types: com.pkg.X cannot be converted to com.pkg.entity.X":
  - This means DUPLICATE CLASSES exist in different packages
  - Check which package repositories and services ACTUALLY use
  - Fix the IMPORTS in services to use the correct package, OR
  - Delete the duplicate class in the wrong package
  - Do NOT just create another duplicate
...
6. For Java: ensure import statements use CORRECT package paths
