...
resource "aws_ecs_task_definition" "backend" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      # Docker TLS configuration. Certs are fetched at startup by
      # backend/scripts/entrypoint.sh from SSM PARAMETER STORE -- that is where the
      # sandbox EC2 user_data publishes them (see ec2-sandbox.tf). These previously
      # pointed at Secrets Manager names that were never created by any terraform or
      # script, so the fetch always failed and sandbox code execution was dead.
      { name = "DOCKER_TLS_ENABLED", value = "true" },
      { name = "DOCKER_TLS_CA_PARAM", value = "/${var.app_name}/docker/ca-cert" },
      { name = "DOCKER_TLS_CERT_PARAM", value = "/${var.app_name}/docker/client-cert" },
      { name = "DOCKER_TLS_KEY_PARAM", value = "/${var.app_name}/docker/client-key" },
