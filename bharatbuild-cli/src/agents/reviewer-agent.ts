/** BharatBuild CLI - Reviewer Agent */
...
export class ReviewerAgent {
  ...
  async review(target: string, opts?: { model?: string; maxTurns?: number }): Promise<void> {
    const config = loadConfig();
    await this._loop.run(
      `Review the following code/changes for bugs, security issues, and quality: ${target}`,
